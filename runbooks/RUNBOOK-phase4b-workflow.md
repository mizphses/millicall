...
curl -s http://192.168.1.3/api/workflows/node-types | jq '.[].type'

# ワークフロー作成（definition は {nodes, edges}）
curl -s -X POST http://192.168.1.3/api/workflows \
...
curl -s -X DELETE http://192.168.1.3/api/workflows/<id> -b cookie.txt   # 204

# 一時的に無効化（Route も disabled になる）
curl -s -X PUT http://192.168.1.3/api/workflows/<id> -b cookie.txt \
