...
詳細な設定手順と公開耐性チェックリストは RUNBOOK-phase6-auth.md（リポジトリ runbooks/ 参照） を参照してください。
...
管理 UI を外部公開する前に以下を確認してください（RUNBOOK-phase6-auth.md § 10（リポジトリ runbooks/ 参照） より）。
...
詳細: RUNBOOK-phase6-auth.md（リポジトリ runbooks/ 参照）
