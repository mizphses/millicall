...
VOICEVOX エンジンの起動方法は環境によって異なります。

**millicallctl（本番）:** `.env` に `MILLICALL_VOICEVOX_ENABLED=true` を設定してから `millicallctl up`（または `millicallctl update`）を実行すると、voicevox エンジンが自動で起動します。

```bash
# ~/millicall/.env
MILLICALL_VOICEVOX_ENABLED=true
```
...
TTS プロバイダのエンジン URL には `http://127.0.0.1:50021` を指定してください。

**手動（開発環境・任意）:** プロファイルを直接指定して起動することも可能です。
...
詳細: RUNBOOK-phase3-ai.md（リポジトリ runbooks/ 参照）
