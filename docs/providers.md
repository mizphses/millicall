...
VOICEVOX を有効化する場合は `docker compose --profile voicevox up -d` で起動します（開発環境）。本番の `docker-compose.prod.yml` には別途設定が必要です（RUNBOOK-phase3-ai.md § TODO（リポジトリ runbooks/ 参照） 参照）。
