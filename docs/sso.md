...
詳細な設定手順と制限事項は RUNBOOK-phase6-auth.md（リポジトリ runbooks/ 参照） を参照してください。
...
> **制限事項**: InResponseTo 未検証（unsolicited 受理）。完全な SP-initiated 検証が必要な場合は RUNBOOK-phase6-auth.md § 11（リポジトリ runbooks/ 参照） を参照。
...
詳細: RUNBOOK-phase6-auth.md § 1（リポジトリ runbooks/ 参照）
