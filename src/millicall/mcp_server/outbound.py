...
# converse システムプロンプト（<end_talk/> 版。旧 [DONE]→[END_CALL]→<end_talk/> と変遷。
# 後方互換で [END_CALL] も検出はされる — millicall.ai.end_talk 参照）。
...
<end_talk/> を発話の末尾に付けてください。
- 目的が達成できない場合（相手が断った等）も、丁寧に終了して <end_talk/> を付けてください。
- <end_talk/> は相手には読み上げられません。会話終了の合図としてだけ使います。
...
def build_converse_system_prompt(purpose: str, key_points: str = "", your_name: str = "") -> str:
    """purpose/key_points/your_name をテンプレート（<end_talk/> 版）に差し込んで合成する。"""
    name_part = ""
    if your_name:
