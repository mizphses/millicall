    ...
    async with sessionmaker() as session:
        ...
        try:
            ...
            if exc.status_code == status.HTTP_429_TOO_MANY_REQUESTS:
                return HTMLResponse(
                    "<html><body><script>"
                    'alert("ログイン試行回数が上限を超えました。しばらく後に再試行してください。");history.back();'
                    "</script></body></html>",
        ...
        if user is None or not password_ok:
            await record_failure(session, ip=ip, username=username, action="mcp_login")
            await record_audit(
                session,
                actor_user_id=None,
                actor_label=username,
                action="mcp_login.failure",
                ip_address=ip,
            )
        ...
        await clear_failures(session, username=username)
        await record_audit(
            session,
            actor_user_id=user.id,
            actor_label=username,
            action="mcp_login.success",
            ip_address=ip,
        )
        await session.commit()
